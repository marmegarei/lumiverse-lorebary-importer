...
const latin1 = new TextDecoder('latin1') // not String.fromCharCode(...bytes): spreading ~200 KB overflows the stack
...
export function pngCardJson(bytes: Uint8Array): string {
  ...
  for (let pos = 8; pos + 12 <= bytes.length; ) {
    ...
    if (type === 'tEXt') {
      ...
      if (key === 'ccv3' || key === 'chara') found[key] = latin1.decode(data.subarray(nul + 1))
    }
    if (type === 'IEND') break
